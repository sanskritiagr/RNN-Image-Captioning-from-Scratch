...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        h0, cache["affine"] = affine_forward(features, W_proj, b_proj) #N,H
        x, cache["embed"] = word_embedding_forward(captions_in, W_embed) #N,T,W
        h, cache["rnn"] = rnn_forward(x, h0, Wx, Wh, b)
        out, cache["tempo"] = temporal_affine_forward(h, W_vocab, b_vocab)
        loss, dout = temporal_softmax_loss(out, captions_out, mask, verbose=False)

        dh, grads["W_vocab"], grads["b_vocab"] = temporal_affine_backward(dout, cache["tempo"])
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(1, max_length):
          embedword = W_embed[prev_word,:]
          next_h, cache["step"] = rnn_step_forward(embedword, next_h, Wx, Wh, b)
